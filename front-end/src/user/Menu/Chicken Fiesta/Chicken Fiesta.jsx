...
const Chicken = () => {
  ...
  const beverage = pizza.filter(
    (each) => each.menu === "Chicken Fiesta" && each.category === category
  );

  return (
    <div className="mt-5 pt-5">
      <h5 className="menu-title mt-1 "><div/><span>Chicken Fiesta</span><div/></h5>
      <div className="container" >
        <section className="card-box my-5 py-5 mt-2 pt-2">
          {beverage.length === 0 ? (
            <h1>We Don't have any {category} items in Chicken Fiesta</h1>
          ) : (
            <>
              {beverage.map((each) => (
